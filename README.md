...
**字号为什么要跟着窗口收**：滚动行是**固定格宽**的，塞不下不会自己缩小、只会换行。实测反推出整行宽 ≈ **9.14 × 字号**，于是字号取 `min(80px, 8.2vw)`；窗口内容区 ≥ 976 px 时正好吃满 80 px。`npm run shot` 会在 17 档窗宽（含每个媒体查询边界两侧）上量「nowrap 实测宽 vs 可用宽」，全部单行不溢出才算过。
