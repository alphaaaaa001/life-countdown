...
- **年份倒计时**：心跳线（全屏背景，基线正好在视口垂直中线）**下方**还有一块，倒计到「明年 1 月 1 日 00:00」。同一套滚轮，但**小时是总小时数**（1 月 1 日那一刻约 8783 小时 ⇒ 4 位数）。右边按钮切换「年份倒计时 ⇄ 剩余生命」，切换时数字抖一下（赛博朋克 glitch）
- **人生进度条**：`已度过年龄 / 预计寿命`
...
npm run verify     # 131 条离线验收断言（心跳线/日期/卫生 68 + 滚轮读秒 63，见下）
npm run verify:dom # 26 条真渲染核对：逐拍比对滚轮显示的数字、切一次模式（会短暂弹窗）
...
### 年份倒计时：同一套滚轮，但小时是 4 位

心跳线是**固定全屏背景、基线正好在视口垂直中线**（尖峰向上挑）。所以"放在心跳线下方"= 放到首屏下半部分 —— 做法是让 `.container` 变成竖向 flex + `min-height: 100vh`，再给这一块 `margin-top: auto`，内容不足一屏时它自然贴着首屏底部，一定落在基线之下。

它和上面那条**共用同一套纯函数**，只有两处不同：
...
**三行滚轮各自独立、都在被持续喂值**（`life` / `year` / `life2`）。这一点是有意的：如果三行共用一组滚轮、切换时重建，切换瞬间就会出现「从 `12 时` 跳到 `8700 时`」的数值突变，滚轮要么疯滚一段、要么得额外补一次落位。各自持续喂值以后，**每行的数值始终连续**，切换只是"换个显示" —— 没有任何东西需要补。

切换时那一下 **glitch 抖动**（`@keyframes odoJitter` 整体横向错位 + `odoRgbSplit` 给每个数字做青/粉分离，180 ms、`steps(1,end)` 跳帧、中途换数）挂在 `.odo-row` 和 `.reel-track > i` 上，**刻意不碰 `.reel-track` 自己的 `transform`** —— 那是滚轮滚动在用的，碰了就会打架。两处细节：`.reel` 有 `overflow:hidden` + 上下遮罩，所以色分离的偏移量只能给 2~3 px（再大就被裁掉、只剩糊）；另外 `prefers-reduced-motion: reduce` 下直接换数、不抖。
...
│   ├─ 年份倒计时（心跳线下方，4 位小时）+ 切换按钮的 glitch 动效
│   └─ 倒计时 / 年龄 / 进度 / 健康提示
...
│   ├─ verify-countdown-dom.js  真渲染核对：逐拍比对滚轮显示的数字、切一次模式（npm run verify:dom）
