...
- **年份倒计时（另一「页」）**：右上角按钮切过去 —— 像翻页一样，生命页那些卡片整体收起，换成「距离明年 1/1 00:00」的大卡 + **今年进度**。同一套滚轮，但**小时是总小时数**（1 月 1 日那一刻约 8783 小时 ⇒ 4 位数）。切页时数字抖一下（赛博朋克 glitch）
- **人生进度条**：`已度过年龄 / 预计寿命`
...
npm run verify     # 138 条离线验收断言（心跳线/日期/卫生 75 + 滚轮读秒 63，见下）
npm run verify:dom # 27 条真渲染核对：逐拍比对滚轮显示的数字、切一次页（会短暂弹窗）
npm run figure     # 重新生成 docs/ecg-waveform.svg
npm run shot       # 重新生成 docs/screenshot.png + 两行读数的 17 档版式体检（需图形界面）
...
`scripts/verify-odometer.mjs`（另 63 条）专验滚轮读秒的纯函数：从 `script.js` 抽同一块 `ODO_PURE_MATH`，模拟从「1 天 02:03:04」逐秒倒着走完 **93,784 秒**，断言 6 根滚轮**每一位显示的永远是真实数字**、每次步进恰好 1 格（含十位回绕），并核对 `odometer-preview.html` 里那份拷贝与正式版**逐字一致**。

其中一组还专门仿真**年份行**：从「1001:00:00」倒着走 3 小时（稳跨千位 1→0），断言 4 位小时 + 分 + 秒共 8 根滚轮、10806 拍里显示的数字 100% 等于真值、最大步进仍是 1 格。

**圈长不写死**：每位滚轮"印几个数字"由**该位的取值范围**推出来（`ODO_FIELD_MAX` → `odoMaxDigitAt` → `odoCycleFor`）：

| 字段 | 取值范围 | 各位圈长 |
|---|---|---|
| 生命行的时 | 0–23 | 十位 **3**、个位 10 |
| 分 / 秒 | 0–59 | 十位 **6**、个位 10 |
| 年份行的时 | 0–8784 | 千位 **9**、百/十/个位 10 |

最早这些数字是写死的（`if (group === 'hours') return 3;`）。加"今年剩余"那一行时它就不够用了 —— 与其再写一份，不如让它从取值范围自己推；**断言里同时钉住"新逻辑必须复现旧的那三个值"**，这就是重构的安全绳。

其中一组专盯「大跳落位」的判定：早先写的是 `delta >= cycle`（想表达"一次跳了整圈以上就别滚"），可 `odoReelDelta` 末尾带 `% cycle`，返回值天生被封在 `[0, cycle-1]` —— **那条分支从来没执行过**。改成按时间差判定（距上次喂值超过 3 s 就当作休眠唤醒 / 页面挂起，直接落位不滚），并加了穷举断言把「旧门槛数学上不可达」钉死。

**⚠️ 但时间差判据有个盲区，做年份行时正好撞上**：它判的是「距上次喂值的毫秒差」，看不见**时间没跳、数值跳了**的场合 ——

| 场合 | 时间差 | 数值跳变 | 时间差判据能挡吗 |
|---|---|---|---|
| 睡眠唤醒 / 页面挂起 | 很大 | 有 | ✅ 能 |
| **跨年那一刻**（`0000:00:00` → `8783:59:59`） | 只隔 1 秒 | 极大 | ❌ **挡不住** |
| **改设置 / 重新设置** | 只隔 1 秒 | 大 | ❌ **挡不住** |

⇒ 这三种情况必须在**代码自己知道**的时刻显式调 `odoSnapRow()` 落位，不能全指望一条通用的时间差判据。这就是为什么 `script.js` 里会有这么一行：

```js
if (odoYearStamp !== null && year !== odoYearStamp) odoSnapRow('year');   // 跨年
```
...
右上角那个按钮（主题按钮正下方）**切页**：生命页 ⇄ 年份页。切过去像翻了页 —— 生命页那些卡片整体收起，只剩「距离今年结束」大卡 + **今年进度** + 重新设置。

**位置**：年份页的内容钉在首屏底部。心跳线是**固定全屏背景、基线正好在视口垂直中线**（尖峰向上挑），所以"在心跳线下方"= 放到首屏下半部分 —— 做法是 `.container` 竖向 flex + `min-height: 100vh`，年份页那一块 `margin-top: auto`。

它和生命行**共用同一套纯函数**，只有两处不同：

| | 余生倒计时 | 年份倒计时 |
|---|---|---|
| 终点 | 出生日期 + 预计寿命 | **明年 1 月 1 日 00:00**（本地）|
| 小时 | `0–23` ⇒ 2 位 | **总小时数 `0–8784`** ⇒ 4 位（千位只用 0–8）|
| 天 | 静态大数字 | 没有（时/分/秒就够了）|

**两行滚轮各自独立、都在被持续喂值**（`life` / `year`）。这一点是有意的：如果两行共用一组滚轮、切页时重建，切换瞬间就会出现「从 `12 时` 跳到 `8700 时`」的数值突变，滚轮要么疯滚一段、要么得额外补一次落位。各自持续喂值以后，**每行的数值始终连续**，切页只是"换个显示" —— 没有任何东西需要补。

切换时那一下 **glitch 抖动**（`@keyframes odoJitter` 整体横向错位 + `odoRgbSplit` 给每个数字做青/粉分离，180 ms、`steps(1,end)` 跳帧、中途换页）挂在 `.odo-row` 和 `.reel-track > i` 上，**刻意不碰 `.reel-track` 自己的 `transform`** —— 那是滚轮滚动在用的，碰了就会打架。三处细节：类挂在 `body` 上（隐藏那一页不参与渲染，只有即将露脸的那页会抖）；`.reel` 有 `overflow:hidden` + 上下遮罩，所以色分离的偏移量只能给 2~3 px（再大就被裁掉、只剩糊）；`prefers-reduced-motion: reduce` 下直接换页、不抖。

**跨年那一刻**：数值从 `0000:00:00` 直接跳到 `8783:59:59`，而相邻两次喂值只隔 1 秒 —— 通用的「时间差大跳」判据**看不见**它，所以这里必须显式落位（见上文那张表）。
...
│   ├─ 年份倒计时（另一「页」，4 位小时）+ 右上角切页按钮的 glitch 动效
│   └─ 倒计时 / 年龄 / 进度 / 健康提示
├─ heartbeat-preview.html  心跳线调参预览页（开发用）
├─ odometer-preview.html   滚轮读秒调参预览页（开发用）
├─ scripts/
│   ├─ verify.mjs          验心跳线 / 日期 / 代码卫生 / 预览页一致性（npm run verify）
│   ├─ verify-odometer.mjs 验滚轮读秒纯函数：93,784 秒长跑仿真 + 年份行仿真（npm run verify）
│   ├─ verify-countdown-dom.js  真渲染核对：逐拍比对滚轮显示的数字、切一次页（npm run verify:dom）
...
│   ├─ screenshot.js       起 Electron 截图 + 两行读数 17 档版式体检（npm run shot）
