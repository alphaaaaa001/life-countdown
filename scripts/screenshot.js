...
app.whenReady().then(async () => {
  ...
  for (const w of widths) {
    ...
    const row = await win.webContents.executeJavaScript(`
      (() => {
        // 量一行「不换行时到底要多宽」：克隆一份、把 flex-wrap 关掉再量
        // （块宽 / Range 都会被布局骗到，详见工程笔记）
        const measure = (id) => {
          ...
          const cell = el.querySelector('.reel');
          const cellH = cell ? parseFloat(getComputedStyle(cell).fontSize) * 1.25 : 0;
          const rowH = el.getBoundingClientRect().height;
          return {
            nowrapW: Math.round(nowrapW),
            availW: Math.round(el.clientWidth),
            wrap: nowrapW > el.clientWidth + 1,
            rows: cellH > 0 ? Math.round(rowH / cellH) : 0,
            digitPx: cell ? getComputedStyle(cell).fontSize : ''
          };
        };
        ...
        return {
          ...
          life: measure('odoRow'),        // 老读秒行：天 + 时/分/秒（6 根滚轮）
          year: measure('odoRowYear'),    // 年份行：4 位小时 + 时/分/秒（8 根滚轮）
  ...
  fits.forEach(f => {
    ...
    const seg = (m) => m
      ? `需 ${String(m.nowrapW).padStart(4)}/可用 ${String(m.availW).padStart(4)} ${m.wrap ? '换行!' : '单行'}(${m.digitPx})`
      : '（没有这一行）';
    console.log(`  ${String(f.win).padStart(4)}px  读秒 ${seg(f.life)}  ` +
      `年份 ${seg(f.year)}  溢出 ${String(f.overflowPx).padStart(3)}px  年份字号 ${f.yearPx}`);
  });
  const real = fits.filter(f => !f.stale);
  const staleCount = fits.length - real.length;
  const rowOk = (m) => !m || (!m.wrap && m.rows <= 1);
  const allFit = real.length > 0 &&
    real.every(f => rowOk(f.life) && rowOk(f.year) && f.overflowPx <= 0);
  console.log(`  实测 ${real.length} 档` + (staleCount ? `，跳过 ${staleCount} 档` : '') +
    `　读秒行 + 年份行 全部单行、不换行、不溢出: ${allFit}` +
